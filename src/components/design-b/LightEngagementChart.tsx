...
export function LightEngagementChart({ data, channel }: { data: DailyMetric[]; channel?: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
      ...
      <p className="text-xs text-gray-400 mb-4">{isYT ? '좋아요 · 댓글 · 공유' : '좋아요 · 댓글 · 공유 · 저장'}</p>
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={formatted}>
          ...
          <Bar dataKey="shares" name="공유" fill="#A5B4FC" stackId="a" radius={isYT ? [4,4,0,0] : undefined} />
          {!isYT && <Bar dataKey="saves" name="저장" fill="#FBCFE8" stackId="a" radius={[4, 4, 0, 0]} />}
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
